...
class TmGoodsSpider(scrapy.Spider):
    ...
    start_urls = [
        ...
        'file:///home/ubuntu/Python/Scrapy/12/topgoods/html/Tmall.htm'
    ]
    # 记录处理的页数
    count = 0
    ...
    def parse(self, response):
        ...
        for div in divs:
            ...
            # scrapy.Request(url=item["GOODS_URL"], meta={'item': item}, callback=self.parse_detail, dont_filter=True)
            url = item["GOODS_URL"]
            scrapy.Request(url, callback=self.parse_detail)
            print(item["GOODS_URL"])
